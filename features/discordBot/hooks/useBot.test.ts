...
import { useBot, useDiscordBot } from "./useDiscordBot";
import * as botStore from "../stores/discordBotStore";
...
describe("useBot", () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(botStore, "useDiscordBotStore").mockReturnValue({
      discordBots: [],
      selectedBotId: null,
      setDiscordBots: setBotsMock,
  ...
  it("should set bots when fetchBots is successful", async () => {
    ...
    waitFor(() => {
      ...
      expect(result.current.discordBots).toEqual(botsData);
    });
  });
  ...
  it("should set bots to an empty array when fetchBots returns no data", async () => {
    ...
    waitFor(() => {
      ...
      expect(result.current.discordBots).toEqual([]);
    });
  });
});
